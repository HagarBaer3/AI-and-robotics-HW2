...
class Node:
    ...
    def __lt__(self, other):
        ...
        return tuple(self.state) < tuple(other.state)   # If ranks are the same, compare by state lexicographically (secondary criterion), used by heapdict to prioritize by rank (cost)
...
class RCSPlanner(object):
    ...
    def plan(self):
        ...
        while openList:
            ...
            if np.array_equal(currentNode_state, env.goal):
        ...
        if len(plan) > 0:
            ...
            print("Total Plan:")
            print (f"Initial state: {np.array(plan[0])}")
            for cnt in range(len(plan) - 1):
                distance = np.linalg.norm(np.array(plan[cnt + 1]) - np.array(plan[cnt]))
                step_resolution = 'Coarse' if (cnt < len(plan) - 1 and distance > np.sqrt(2)) else 'Fine'           
                x_loc,y_loc = plan[cnt + 1]
            ...
            print(f" coase = {float(100*13/14)}, fine = {100/14}")
        else:
