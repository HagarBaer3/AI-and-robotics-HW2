...
class RRTPlanner(object):
    ...
    def plan(self):
        ...
        for cnt in range(runs_num):
            print(f" Run {cnt}, delta  time ={time.time()-start_time}")
            ...
            while not self.tree.is_goal_exists(state = goal_state):
                ...
                if env.edge_validity_checker(nearest_neighbor_state, added_state):
                    ...
                    if np.linalg.norm(added_state - goal_state) < 1e-3:
                        break
            print(f"finished with {iter_num} iterations")
