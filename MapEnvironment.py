...
class MapEnvironment(object):
    ...
    def visualize_map(self, ext_mode, goal_prob, planner, show_map=False, plan=None, tree_edges=None, expanded_nodes=None, eta = None):
        ...
        if show_map:
            ...
            filename_1 = planner + '_' + 'eta' + str(eta) + '_' + ext_mode + '_' + str(goal_prob) + '.png'
            #plt.savefig('{}'.format(filename_1))
            #plt.savefig('map.png')
            plt.savefig(filename_1)

        return plt
    def visualize_map1(self, show_map=False, plan=None, tree_edges=None, expanded_nodes=None):
        '''
        Visualize map with current state of robot and obstacles in the map.
        @param show_map If to show the map or save it.
        @param plan A given plan to draw for the robot.
